...
def fig_pipeline():
    ...
    ax.text(50, 92, "6단계 자동화 흐름 — 회계사가 판단만 입력하면 자동계산 엔진이 세무조정을 확정",
            ha="center", fontproperties=_KFONT_B, fontsize=11.5, color=NAVY)
    ax.text(50, 12, "④ AI 검토보조는 '선택' — 세무조정 계산(⑤)은 AI 없이 자동계산 엔진만으로 완결",
...
def fig_architecture():
    ...
    _box(ax, 4, 74, 24, 16, "회계 프로그램 자료\n더존 Smart A · WEHAGO\n(재무제표·분개장·고정자산 등 8종)",
         *BOX_GRY, fs=9)
    # 표준화
    _box(ax, 38, 74, 24, 16, "표준화 정리 과정\n파일형식·글자깨짐·제목행·음수표기\n자동 인식·정리",
         *BOX_BLUE, fs=9, bold=True)
    # 자동계산 엔진 (핵심)
    _box(ax, 20, 44, 60, 20,
         "자동계산 엔진  (핵심 · 법령 규칙 기반)\n분개장 일괄 집계 · 적수 일별계산 · 손금불산입/익금산입 산식\n"
         "4만 건도 1초 내 확정",
    ...
    ax.text(83.5, 59, "선택 항목만", fontproperties=_KFONT, fontsize=7.5, color="#8a6d1f",
            rotation=90, va="center")
    ax.text(50, 96, "시스템 구조 — 자동계산 엔진이 세무판단을 확정하고, AI는 보조만 (설계 원칙)",
            ha="center", fontproperties=_KFONT_B, fontsize=11.5, color=NAVY)
...
def fig_engine():
    fig, ax = _fig(11, 4.4)
    ax.text(50, 94, "자동계산 엔진 — 분개장을 한 번에 집계, 적수는 일별 재구성",
            ha="center", fontproperties=_KFONT_B, fontsize=11.5, color=NAVY)
    _box(ax, 3, 40, 20, 30, "분개장\n(전표·날짜·계정\n차·대변·증빙)", *BOX_GRY, fs=9)
    _box(ax, 30, 62, 30, 12, "일괄 집계\n접대비·벌과금·법인세·평가손익·이자",
         *BOX_TEAL, fs=9, bold=True)
...
def fig_coverage():
    ...
    a1.axis("off")
    items = [
        ("모든 조정 = 법령 근거", "조문번호까지 필수 명시"),
        ("금액·세액 확정 = 자동계산 엔진", "AI는 확정 안 함"),
        ("감사추적 = 분개 행 단위", "'왜 자동조정 되었나' 판정사유"),
        ("실제 데이터 재검증", "티엘 WEHAGO 49,060 분개 일치"),
        ("품질", "자동 검증 299건 통과"),
    ]
    y = 0.9
    for t, d in items:
...
def build():
    ...
    pdf.cell(0, 8, "회계자료를 자동계산 엔진으로 자동 세무조정 — 모든 조정은 국가법령정보 기반, 감사추적은 분개 행 단위",
    ...
    callout(
        pdf, "한 문장 요약",
        "더존 Smart A·WEHAGO 재무자료를 자동으로 읽어들여 분개장 일괄 집계 + 법령 산식 자동계산 엔진으로 "
        "세무조정 금액·과세표준·세액을 확정하고, 사내 AI가 애매한 분개의 "
        "검토를 보조하는 시스템입니다. AI는 세무판단을 확정하지 않으며 — 초안·근거·검토포인트 "
    ...
    table(
        ...
        [
            ...
            ["핵심 엔진", "분개장 일괄 집계 · 적수 일별계산 · 손금불산입/익금산입 법령 산식 (규칙 기반)"],
            ["왜 신뢰 가능", "모든 조정 = 국가법령정보 조문 근거 · 분개 행 단위 감사추적 · 실제 데이터 재검증"],
            ["구현 방식", "사내 PC 설치형 프로그램(브라우저 화면) · 법령 규칙 자동계산 엔진 · 보조 AI(선택)"],
        ],
        widths=[26, 114])

    callout(
        pdf, "회계사가 5분 안에 확인할 것",
        "① 세무조정 금액·세액을 AI가 아니라 자동계산 엔진이 확정하는가  "
    ...
    body(pdf,
         ...
         "자동계산 엔진이 법령 산식으로 세무조정을 확정합니다. AI 검토보조(④)는 선택 단계로, "
         "이를 건너뛰어도 세무조정 계산(⑤)은 자동계산 엔진만으로 완결됩니다.")
    img(pdf, p_pipeline)
    body(pdf,
         "회계 프로그램에서 내려받은 자료(8종)는 항상 깨끗하지 않습니다 — 겉은 엑셀이지만 내부 형식이 다른 파일, "
         "글자 깨짐, 제목행, 음수 표기(△·괄호·전각), 합계행, 회사별 계정명 차이를 표준화 정리 과정이 "
         "하나의 관문에서 흡수합니다. 원본 자료의 줄 위치는 감사추적용으로 보존됩니다.")

    # ── 3. 아키텍처 ──────────────────────────────────────────
    pdf.add_page()
    h_section(pdf, "2. 시스템 구조 — 자동계산 엔진이 확정, AI는 보조")
    img(pdf, p_arch)
    callout(
        pdf, "왜 이 구조가 회계법인에 적합한가",
        "세무조정 금액·한도·세액 계산은 자동계산 엔진만 담당하고, AI는 대량 분개 판단·금액 확정에 "
        "일절 관여하지 않습니다. AI는 회계사가 선택한 소수 고위험 거래의 요약·검토메모 초안만 "
        "생성하며, 결과는 계산에 반영되지 않고 감사추적에 보존됩니다. 즉 'AI의 착오가 금액을 바꾸는' 경로가 "
        "원천 차단됩니다.")

    # ── 4. 규칙엔진 ──────────────────────────────────────────
    pdf.add_page()
    h_section(pdf, "3. 자동계산 엔진 — 규칙 기반 세무조정 계산")
    img(pdf, p_engine)
    table(
        pdf, ["항목", "계산 기준", "법령"],
        [
    ...
    callout(
        pdf, "검증 가능성 · 재현성",
        "회사 재무수치는 제출한 원본 자료, 세무조정은 법령 조문으로 추적 가능합니다. 계산 항목을 선택하면 "
        "'왜 자동조정 되었나'(판정사유) + 적용 산식 + 근거 분개 + 엑셀 내려받기까지 단계별로 펼쳐 볼 수 있습니다. "
        "실제 데이터 재검증(티엘 WEHAGO 분개 49,060건 핵심수치 일치)과 자동 검증 299건으로 품질을 보증합니다.")

    # ── 6. 데모 산출 (한빛정밀) ──────────────────────────────
    pdf.add_page()
    h_section(pdf, "5. 데모 산출 예시 — 한빛정밀㈜ (가상)")
    body(pdf,
         "온라인 데모에서 면접관이 자료 불러오기 없이 '데모 데이터 불러오기'만 누르면, 아래 가상 회사의 "
         "분개장이 자동계산 엔진을 통과해 세무조정 후보와 근거가 즉시 생성됩니다. (전부 가상 데이터)")
    table(
    ...
    callout(
        ...
        "소득처분(귀속자)·개별 사실관계·최신 예규는 회계사 확인이 필요합니다. 온라인 데모는 반드시 "
        "가상 데이터만 사용하며, 실제 고객 세무자료는 사내 PC에서만 실행(외부 미전송)을 전제로 합니다.",
        border=(180, 60, 60))
